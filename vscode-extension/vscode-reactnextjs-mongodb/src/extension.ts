...
function getApiUrl(): string {
  ...
  const apiUrl = configuration.get<string>("apiUrl")?.trim();
  return apiUrl && apiUrl.length > 0 ? apiUrl : DEFAULT_API_URL;
}

function getDefaultDownloadAction(): DownloadActionOrPrompt {
  const configuration = vscode.workspace.getConfiguration(CONFIG_SECTION);
  const action = configuration.get<string>("defaultDownloadAction", "prompt");
...
function getNamedFolderName(defaultFileName: string): string {
  const configuration = vscode.workspace.getConfiguration(CONFIG_SECTION);
  let folderName = configuration.get<string>("namedDownloadFolderName", "${fileName}");
