...
// Configuration
const CONFIG = {
  API_URL: process.env.J2CODE_API_URL || "https://www.j2code.app/api/j2code/angular-firestore",
...
const TOOLS: ToolDefinition[] = [
  {
    name: "generate-angular-firestore-app",
    description:
      "Generate a production-ready Angular Firestore business application from sample JSON business data. Sends your sample JSON business record to the J2Code API which generates complete, deployable source code.",
    inputSchema: {
      type: "object",
      properties: {
        sampleJsonData: {
...
async function runServer() {
  ...
  server.setRequestHandler(
    ...
    async (request: CallToolRequest) => {
      ...
      try {
        switch (toolName) {
          case "generate-angular-firestore-app":
            return {
              content: [
                await generateApp(
  ...
  console.error("MCP J2Code Angular Firestore server started successfully");
}
