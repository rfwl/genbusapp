/* Copyright (c) 2025 j2code.app - Licensed under the MIT License (see LICENSE file for details) */
  ...
  import { StudentAddressRecordChildEditorFormComponent } from '../editor-form/student-address-record-child-editor-form.component'



  @Component({
    selector: "student-address-record-child-editor-aggrid", 
templateUrl: './student-address-record-child-editor-aggrid.component.html', 

styleUrls: ['./student-address-record-child-editor-aggrid.component.scss'] 

}) 

  export class StudentAddressRecordChildEditorAgGridComponent implements OnInit {
    ...
    outputJsonString(){
      ...
      show_CodeOutput_Modal(this.modalService, 'StudentAddressRecords', str, 'modal-xl')
    }
    ...
    initEditorAgGrid(){
      ...
      this.editor_agGrid_defaultColDef = { sortable: true, filter: true, resizable: true, maxWidth:1000, suppressMovable: true }
    ...
    editor_agGrid_onGridReady(params: { api: any; columnApi: any }) {
      ...
      this.editor_agGrid_gridApi?.updateGridOptions({rowData: this.editor_agGrid_rowData})

    }

    colDefs =[
      { cellRenderer: 'openFieldEditorCellRenderer', width: 60, tooltipField: 'id', filter: false, sortable: false,editable: false, resizable: false,
        cellRendererParams: {
          icon: 'edit', display: '',  tooltip: 'Edit',
          parentComponent: this, parentFunction: this.showUpdateForm,
        }
      },
      { cellRenderer: 'openFieldEditorCellRenderer', width: 60, filter: false, sortable: false,editable: false, resizable: false,
        cellRendererParams: {
...
{ field: "State", tooltipField:  "State", headerName: "State", flex: 1 }, 
{ field: "Postcode", tooltipField:  "Postcode", headerName: "Postcode", flex: 1 }, 
{ field: "Country", tooltipField:  "Country", headerName: "Country", flex: 1 }, 
    ...
    showCreateForm(field: string, rowIndex: number = undefined) {
      ...
      let modalRef = this.modalService.show(StudentAddressRecordChildEditorFormComponent, config)
