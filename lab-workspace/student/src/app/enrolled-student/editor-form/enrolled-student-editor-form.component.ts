/* Copyright (c) 2025 j2code.app - Licensed under the MIT License (see LICENSE file for details) */
  ...
  import { EnumSelectorOptionSource } from '../enrolled-student.model'
...
import { ContactChildEditorFormComponent } from "../contact/editor-form/contact-child-editor-form.component";
  ...
  @Component({
    selector: "enrolled-student-editor-form", 
templateUrl: './enrolled-student-editor-form.component.html', 

styleUrls: ['./enrolled-student-editor-form.component.scss'] 

}) 


  export class EnrolledStudentEditorFormComponent implements OnInit {
    firestore: Firestore
    ...
    outputJsonString(){
      ...
      show_CodeOutput_Modal(this.modalService, 'EnrolledStudent', str, 'modal-xl')
    }
    ...
    get editorFormTitle(): string{
      switch(this.editorOperationType){
        case EnumEditorOperationType.Create:
          return 'Create EnrolledStudent'
        case EnumEditorOperationType.Delete:
          return 'Delete EnrolledStudent'
        case EnumEditorOperationType.Update:
          return 'Update EnrolledStudent'
        default:
          return 'Edit EnrolledStudent'
      }
    }
    ...
    submitEditorForm(editorForm:any){
      ...
      switch(this.editorOperationType){
        case EnumEditorOperationType.Create:
          ...
          // this.store.dispatch(EnrolledStudentActionTypes.createEnrolledStudent({ enrolledStudent: {...this.editedObject } }))
          break
        case EnumEditorOperationType.Delete:
          if(this.parentDataArray && this.parentDataIndex >= 0){
            this.parentDataArray?.splice(this.parentDataIndex,1)
            // moved the next line to Editor AgGrid
            // this.store.dispatch(EnrolledStudentActionTypes.deleteEnrolledStudent({ enrolledStudentId: this.editedObject.id }))
          }
          break
        case EnumEditorOperationType.Update:
          break
